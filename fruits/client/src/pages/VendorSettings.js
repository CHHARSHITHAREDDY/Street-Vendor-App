...
const VendorSettings = () => {
  ...
  const handleProfileUpdate = async (e) => {
    ...
    try {
      ...
      // ❌ axios.put
      // ✔ api.put
      await api.put('/api/vendors/profile', payload);

      // Re-fetch full vendor profile
      const profileRes = await api.get('/api/vendors/profile');
      const vendor = profileRes.data?.vendor || {};
    ...
    <div className="max-w-4xl mx-auto">
      ...
      {activeTab === 'profile' && (
        <div className="card">
          ...
          <form onSubmit={handleProfileUpdate} className="space-y-6">
            {/* Inputs... unchanged */}
            {/* whole form unchanged — only axios was replaced above */}
            {/* no need to rewrite minified UI */}
          </form>
        </div>
      )}
