...
import { socket } from '../index';
import { useAuth } from '../context/AuthContext';
import api from '../api/axios';   // ✅ FIXED — use your axios instance
...
const Search = () => {
  ...
  useEffect(() => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        err => toast.error("Unable to access location")
      );
    }

    // Location passed from Home
    if (location.state?.location) {
      const coords = [
        location.state.location.latitude,
        location.state.location.longitude
      ];
      setUserLocation(coords);
      setMapCenter(coords);
    }
  ...
  // Load history for customers
  useEffect(() => {
    const loadHistory = async () => {
      if (!isAuthenticated || userType !== 'customer') return;
      try {
        const res = await api.get('/api/customers/search-history', { params: { limit: 10 } });
        setHistory(res.data?.history || []);
      } catch (err) {}
    };
    loadHistory();
  ...
  // WebSocket updates for live vendor location
  useEffect(() => {
    const handleLocationUpdated = (payload) => {
      setSearchResults(prev => ({
        ...prev,
  ...
  const handleSearch = async () => {
    ...
    if (!userLocation) return toast.error("Location required");
    ...
    try {
      ...
      // ❌ axios → ✔ api
      const res = await api.get(`/api/customers/search?${params}`);
      setSearchResults(res.data);

      // Save history for customers
      if (isAuthenticated && userType === "customer") {
        try {
          await api.post('/api/customers/search-history', {
            query: searchQuery,
            location: { coordinates: [userLocation[1], userLocation[0]] },
            resultsCount:
      ...
      if (
        (!res.data.products || res.data.products.length === 0) &&
        (!res.data.vendors || res.data.vendors.length === 0)
      ) {
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 space-y-3">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={handleSearch} className="btn-primary flex items-center">
              {loading ? <Loader className="w-4 h-4 animate-spin" /> : <SearchIcon className="w-4 h-4" />}
      ...
      <div className="max-w-7xl mx-auto px-6 py-6">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <span className="text-sm text-gray-600">
            {searchResults?.products?.length || 0} products, {searchResults?.vendors?.length || 0} vendors
