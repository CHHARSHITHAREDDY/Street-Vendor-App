...
import VendorSettings from './VendorSettings';
import { socket } from '../index';
import api from '../api/axios';   // ✅ USE YOUR CONFIGURED API HERE
...
const DashboardOverview = () => {
  ...
  const toggleAvailability = async () => {
    try {
      ...
      // ❌ axios.put → ✔ api.put
      const res = await api.put('/api/vendors/availability', {
        isAvailable: next
      });
...
const ProductsManagement = () => {
  ...
  const toggleProductAvailability = async (productId, isAvailable) => {
    try {
      // ❌ axios.put → ✔ api.put
      await api.put(`/api/products/${productId}/availability`, { isAvailable });

      setProducts(prev =>
        prev.map(p => (p._id === productId ? { ...p, isAvailable } : p))
      );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <Link to="/vendor-dashboard/products/new" className="btn-primary flex items-center space-x-2">
          <Plus className="w-4 h-4" />
          <span>Add Product</span>
        </Link>
      </div>
      ...
      <div className="flex space-x-4">
        {['all', 'available', 'unavailable'].map(type => (
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              filter === type ? 'bg-primary-100 text-primary-700' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
            }`}
          >
          ...
          <div key={product._id} className="card">
            <div className="flex justify-between items-start mb-3">
              ...
              <div className="flex items-center space-x-2">
                <button
                  onClick={() => toggleProductAvailability(product._id, !product.isAvailable)}
                  className={`p-1 rounded ${product.isAvailable ? 'text-green-600 hover:bg-green-100' : 'text-gray-400 hover:bg-gray-100'}`}
                >
                  {product.isAvailable ? <ToggleRight className="w-5 h-5" /> : <ToggleLeft className="w-5 h-5" />}
                </button>
      ...
      {products.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">No products found</h3>
          <p className="text-gray-600 mb-4">Get started by adding your first product</p>
          <Link to="/vendor-dashboard/products/new" className="btn-primary">
            Add Product
          </Link>
        </div>
      )}
...
const ProductForm = () => {
  ...
  const categories = ['fruits', 'vegetables', 'dairy', 'meat', 'beverages', 'snacks', 'bakery', 'grocery', 'other'];

  const units = ['kg', 'lb', 'piece', 'dozen', 'liter', 'gallon', 'pack', 'bunch', 'bag'];
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            ...
            <select name="unit" value={formData.unit} onChange={handleChange} required className="input-field">
              {units.map(unit => (
                <option key={unit} value={unit}>
                  {unit.toUpperCase()}
                </option>
          ...
          <div>
            ...
            <input type="number" name="quantity" value={formData.quantity} onChange={handleChange} required className="input-field" />
          </div>
        </div>
        ...
        <div className="flex items-center space-x-6">
          ...
          <label className="flex items-center">
            <input type="checkbox" name="local" checked={formData.local} onChange={handleChange} />
